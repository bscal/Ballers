            ...
            foreach (Frame frame in Frames)
            {
                for (int i = 0; i < frame.transform.Find("Chunks").childCount; i++)
                {
                    ...
                    if (o.GetComponent<Renderer>() && IsEnabledForEditing)
                        UnityEditor.EditorUtility.SetSelectedRenderState(o.GetComponent<Renderer>(), EditorSelectedRenderState.Hidden);
                    else
                        UnityEditor.EditorUtility.SetSelectedRenderState(o.GetComponent<Renderer>(), EditorSelectedRenderState.Highlight);
                }
            }
